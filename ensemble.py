# ensemble.py
"""
Ensemble pipeline (MLP + RandomForest + optional XGBoost).
Reads CNN models from models/cnn/ (if use_cnn True) and image features from dataset.
Saves ensemble models to models/ensemble/ and results to models/results/ensemble/
Outputs JSON in Format A for each model key.
"""
...
# Image feature extractors (same as previous)
...
def extract_image_features(image_paths, bins=32, hog_ppc=(16,16)):
    ...
    for p in image_paths:
        ...
        feats.append(vec)
    maxlen = max((v.shape[0] for v in feats), default=0)
...
def run_ensemble(socketio, processed_dir, models_dir, results_dir, use_cnn=True, use_image_feats=True, n_pca=150):
    ...
    try:
        ...
        results_ensemble_dir = os.path.join(results_dir)
            ...
            if use_cnn and cnn_models:
                ...
                if cnn_names:
                    try:
                        ...
                        _emit(socketio, "ensemble_log", {"message": f"⚠️ Could not concat CNN preds: {e}"})
            if not feats_list:
        ...
        # Evaluate & produce Format A per model
        ...
        def eval_and_report(model_obj, name):
            try:
                ...
                p_macro, r_macro, f_macro, _ = precision_recall_fscore_support(y_test_mapped, ypred, average='macro', zero_division=0) if y_test_mapped.size else (0.0,0.0,0.0,[])
                ...
                results_report[name] = {
                    ...
                    "confusion_matrix": cm,
                    "model_path": os.path.join(models_ensemble_dir, f"{name}.pkl") if name in trained_models else None,
                    "created_at": time.strftime("%Y-%m-%d %H:%M:%S")
                }
                _emit(socketio, "ensemble_log", {"message": f"   • {name} — Acc: {acc:.4f}, F1: {f_macro:.4f}"})
            except Exception as e:
        ...
        # Save final JSON in Format A: unify under results_ensemble_dir/ensemble_report.json
        out_json = os.path.join(results_ensemble_dir, "ensemble_report.json")
        to_save = {"meta": {"processed_dir": processed_dir, "models_cnn_dir": models_cnn_dir, "created_at": time.strftime("%Y-%m-%d %H:%M:%S")}, "results": results_report}
        with open(out_json, "w") as f:
            json.dump(to_save, f, indent=2)

        # optional confusion plot for stacking
        try:
        ...
        # ==========================================
        # SAVE ENSEMBLE REPORT (dashboard)
        # ==========================================
        ensemble_report = {
            "created_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        save_path = os.path.join(results_ensemble_dir, "ensemble_report.json")

        with open(save_path, "w") as f:
            json.dump(ensemble_report, f, indent=2)

        _emit(socketio, "ensemble_done", {
            "message": "Ensemble selesai — ensemble_report.json dibuat."
        })
